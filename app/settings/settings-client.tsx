...
export function SettingsClient({ userId, email, profile, stats }: Props) {
  ...
  const [ratingTennis, setRatingTennis] = useState(profile.ratingTennis);
  const [ratingPickleball, setRatingPickleball] = useState(profile.ratingPickleball);
    ...
    try {
      ...
      const { error } = await supabase
        ...
        .update({
          ...
          rating_tennis: ratingTennis || null,
          rating_pickleball: ratingPickleball || null,
          gender,
        })
        .eq("id", userId);
    ...
    <div className="space-y-8">
      ...
      <section className="bg-panel border border-white/10 rounded-2xl p-6">
        ...
        <div className="space-y-3">
          ...
          <label className="block text-chalk-dim text-xs mb-1">Your ratings</label>
          <div className="grid grid-cols-2 gap-2">
            <div>
              <span className="block text-chalk-dim text-[11px] mb-1">Tennis</span>
              <select
                value={ratingTennis}
                onChange={(e) => setRatingTennis(e.target.value)}
            ...
            <div>
              <span className="block text-chalk-dim text-[11px] mb-1">Pickleball</span>
              <select
                value={ratingPickleball}
                onChange={(e) => setRatingPickleball(e.target.value)}
                className="w-full bg-court-deep border border-white/10 rounded-lg px-2 py-2 text-sm"
              >
                <option value="">Not set</option>
                {RATINGS.map((r) => <option key={r} value={r}>{r}</option>)}
          ...
          {/* Two ratings because they are often genuinely different: the scoring
              uses whichever matches the sport being played. */}
          <p className="text-chalk-dim text-xs mb-4 mt-1">
            Shown next to your name in the standings. Every league mixes all ratings, so
            this is how opponents know what kind of match to expect &mdash; and it decides
            how much a win over you is worth.
          </p>

          <label className="block text-chalk-dim text-xs mb-1">Gender</label>
          <select
            value={gender}
          ...
          <p className="text-chalk-dim text-xs mb-4 mt-1">
            Decides which leagues you can join. Changing it does not move you out of
            leagues you are already in.
          </p>
